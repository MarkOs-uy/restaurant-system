...
export default function TablesPage({ isAdmin }: { isAdmin: boolean }) {
  ...
  const getTableColor = (table: Table) => {
    ...
    if (table.order_status === "IN_PROGRESS") return "#251bde"   // Azul vibrante
    if (table.order_status === "READY") return "#10b981"  // Verde esmeralda
    if (table.order_status === "CLOSED") return "#8b5cf6" // Violeta místico
    return "#637381"
  }
    ...
    <div className="tables-page">
      ...
      {isAdmin && editMode && (
        <div className="layout-editor">
            <div className="layout-editor__group">
            <span>Plano</span>
            <input
              type="number"
              value={layout.width}
          ...
          <div className="layout-editor__group">
            ...
            <input
              ...
              onChange={(e) => setLayout({ ...layout, grid_size: Number(e.target.value) })}
              style={{ width: 80 }}
            />
            </label>

            <label className="layout-editor__checkbox">
              <input
                type="checkbox"
                checked={layout.snap_to_grid}
                onChange={(e) => setLayout({ ...layout, snap_to_grid: e.target.checked })}
              />
              Ajustar a la grilla
            </label>
          </div>
      ...
      {editMode && (
        <div className="tables-edit-notice">
          <strong>🛠 Modo edición activo</strong>
          <span>
            Arrastra las mesas para moverlas.
            Clic derecho para desactivar.
          </span>
        </div>
      )}

      {showForm && (
        <div className="table-create-card">
          <div className="table-create-card__header">
            <h3>Nueva mesa</h3>
          </div>

          <div className="table-create-card__fields">
            <input
              type="number"
              min="1"
              placeholder="Número"
            ...
            <select
              ...
              onChange={(e) => setNewTableForm({...newTableForm, shape: e.target.value})}
            >
            ...
            {newTableForm.shape === "rectangle" && (
              <select
                ...
                onChange={(e) => setNewTableForm({...newTableForm, orientation: e.target.value as "horizontal" | "vertical"})}
                style={{ marginLeft: 10 }}
              >
          ...
          <div className="table-create-card__actions">
            <button
              type="button"
              className="btn btn-primary"
              onClick={createTable}
      ...
      <div className="restaurant-floor-scroll">
        <div
          ref={floorRef}
          className="restaurant-floor"
          style={{
            width: layout.width,
            ...
            backgroundSize:
              `${layout.grid_size}px ${layout.grid_size}px,
              ${layout.grid_size}px ${layout.grid_size}px${
                layout.background_image
                  ? ", contain"
                  : ""
          ...
          {tables.filter(t => t.active).map(t => {
            ...
            return (
              ...
              >
                ...
                <div className="floor-table__content">
                  <strong>
                    {t.number}
                  </strong>
                  {t.order_status && (
